...
class SharedFiles
  ...
  def self.register(some_files, some_port) # Expects an array like ["/tmp/a.txt", "/c.doc"]
    post('/shared_files', :body => [serialize_file_array(some_files), "port=#{some_port}"].join("&") )
  end
  
  def self.monitor(some_directory, some_port)
